...
@section('page_title', __('site.action.edit_entity', ['entity' => __('site.options.single')]))

@section('content')
    <div class="container">
        <div class="row">
        <h1>{{__('site.action.edit_entity', ['entity' => __('site.options.single')])}}</h1>
            <div class="col-md-12">
                <form method="post" enctype="multipart/form-data" action="{{route('options.update',[$model])}}">
                    @method('POST')
                    @csrf
                    <div class="mb-3">
